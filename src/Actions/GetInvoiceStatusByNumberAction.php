...
class GetInvoiceStatusByNumberAction
{
    public function __construct(
        protected PeppolGatewayService $service,
    ) {}

    public function execute(string $invoiceNumber): InvoiceStatus
    {
        $response = $this->service->getInvoiceStatusByNumber($invoiceNumber);

        if (($response['_status'] ?? null) === 404) {
            throw InvoiceException::notFound($invoiceNumber);
        }

        $status = InvoiceStatus::fromResponse($response);

        InvoiceStatusRetrieved::dispatch($invoiceNumber, $status);
